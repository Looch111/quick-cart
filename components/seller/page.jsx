...
const SellerDashboard = () => {
  ...
  const { data: allOrdersData, loading: ordersLoading } = useCollection('orders');
  const { data: productsData, loading: productsLoading } = useCollection('products');
  const { data: platformSettings, loading: settingsLoading } = useDoc('settings', 'platform');
  ...
  const loading = ordersLoading || productsLoading || settingsLoading;

  const allOrders = useMemo(() => {
    if (!allOrdersData) return [];
    return allOrdersData.map(o => ({...o, _id: o.id, date: o.date?.toDate ? o.date.toDate() : new Date(o.date) }));
  }, [allOrdersData]);

  const products = useMemo(() => {
      if (!productsData) return [];
      return productsData.map(p => ({...p, _id: p.id }));
  }, [productsData]);

  useEffect(() => {
    if (userData && allOrders && products && !loading && platformSettings) {
  ...
  }, [userData, allOrders, products, loading, platformSettings]);
